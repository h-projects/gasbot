  ...
  if (!message.content.startsWith("h!eval")) {
    ...
    function gDetected() {
      ...
      message.delete().catch(() => {
        console.error("Can't delete messaqe");
      });
      ...
      message
        .reply("don't use the bad letter!")
        .then((message) => {
          message.delete({ timeout: 4000 });
        })
        .catch(() => {
          console.error("Can't post messaqe");
        });
      loqChannel.send(loqEmbed).catch();
      if (message.guild.id != "701809497206685796") {
        gasserverlog.send(qlobalLoqEmbed).catch(() => {
          console.error("Central G.A.S #loqs channel not found");
        });
      }
    }
